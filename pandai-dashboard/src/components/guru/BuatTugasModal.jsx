...
const BuatTugasModal = ({ isOpen, onClose }) => {
    ...
    const [selectedQuestions, setSelectedQuestions] = useState([2]); // Mock: Question with ID 2 is selected
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-slate-900/40 backdrop-blur-sm animate-in fade-in duration-300">
            <div className="bg-white w-full max-w-[1200px] h-[90vh] rounded-[16px] shadow-[0px_25px_50px_-12px_rgba(0,0,0,0.25)] border border-[#3713EC]/10 flex flex-col overflow-hidden relative">
                ...
                <div className="flex flex-1 overflow-hidden">
                    ...
                    <div className="w-[460px] border-r border-[#F1F5F9] p-8 overflow-y-auto bg-white flex flex-col gap-8">
                        <div className="flex flex-col gap-6">
                            ...
                            <div className="flex flex-col gap-2">
                                ...
                                <div className="bg-[#F8FAFC] border border-[#E2E8F0] rounded-lg overflow-hidden flex flex-col">
                                    {/* Toolbar */}
                                    <div className="px-3 py-2 border-b border-[#E2E8F0] bg-white flex items-center gap-2">
                                        <button className="p-1.5 hover:bg-slate-100 rounded text-[#1E293B]"><Bold size={16} /></button>
                                        <button className="p-1.5 hover:bg-slate-100 rounded text-[#1E293B]"><Italic size={16} /></button>
                                        <button className="p-1.5 hover:bg-slate-100 rounded text-[#1E293B]"><List size={16} /></button>
                                        <button className="p-1.5 hover:bg-slate-100 rounded text-[#1E293B]"><LinkIcon size={16} /></button>
                                    </div>
                                    <textarea
                    ...
                    <div className="flex-1 bg-[#F8FAFC]/50 p-8 overflow-y-auto flex flex-col gap-6">
                            ...
                            <div className="relative z-10 flex flex-col gap-4">
                                ...
                                <div className="flex items-center gap-2">
                                    <input
                                        ...
                                        className="flex-1 bg-white/20 border border-white/10 rounded-lg px-4 py-3 text-[14px] placeholder-white/60 focus:outline-none focus:bg-white/30 transition-all font-sans"
                                    />
                                    <button className="bg-white text-[#3713EC] px-6 py-3 rounded-lg font-bold text-[14px] shadow-lg shadow-black/10 hover:bg-slate-50 transition-all active:scale-95 shrink-0">
                                        Buat Soal
                                    </button>
                                </div>
                            </div>
                            ...
                            {questions.map((q) => (
                                <div key={q.id} className={cn(
                                    ...
                                    <div className="flex items-center justify-between">
                                        ...
                                        <button className={cn(
                                            "flex items-center gap-2 px-4 py-1.5 rounded-full text-[12px] font-bold transition-all active:scale-95",
                                            q.status === 'selected'
                ...
                {/* Footer */}
                <div className="px-6 py-5 border-t border-[#F1F5F9] flex items-center justify-between bg-white shrink-0">
                    <div className="flex items-center gap-2 text-slate-400">
                        <Info size={14} />
                        <p className="text-[12px] font-medium">Draft disimpan otomatis pada 14:30 WIB</p>
                    </div>
                    <div className="flex items-center gap-4">
                        <button onClick={onClose} className="px-6 py-2.5 text-[16px] font-semibold text-[#475569] hover:bg-slate-50 rounded-lg transition-colors">
                            Batal
                        </button>
                        <button className="bg-[#3713EC] hover:bg-[#2d0fb8] text-white px-8 py-2.5 rounded-lg font-bold text-[16px] shadow-lg shadow-[#3713EC]/20 transition-all active:scale-[0.98]">
                            Simpan & Publikasikan
                        </button>
                    </div>
                </div>
